...
export default class CreateDeltaPackage extends SfCommand<void> {
  ...
  public static readonly flags = {
    ...
    valuecolumn: Flags.string({ char: 'h', summary: messages.getMessage('flags.valuecolumn.summary') }),
  ...
  public async run(): Promise<void> {
    ...
    if (result.records.length < 1) {
      ...
      CreateDeltaPackage.doDelta(simpleGit, sourceFolder, deltaPackageFolder, previousHash, verbose);
    }
  }

  static copyCompleteFolder(sourceFolder: string, deltaPackageFolder: string): void {
    if (fsExtra.existsSync(deltaPackageFolder)) {
      fsExtra.removeSync(deltaPackageFolder);
    }
  ...
  static doDelta(simpleGit: any, sourceFolder: string, deltaPackageFolder: string, previousHash: string, verbose?: boolean): void {
    simpleGit.diffSummary([previousHash, '--no-renames'], (err: any, status: any) => {
      if (err) {
        ...
        if (numOfDiffs > 0) {
          ...
          for (const files of status.files) {
            if (verbose) console.log('File: ' + files.file);
