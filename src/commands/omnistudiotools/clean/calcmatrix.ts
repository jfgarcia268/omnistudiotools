...
export default class CleanCalcMatrix extends SfCommand<void> {
  ...
  private async updateOldCalMatrixVersion(matrixid: string, conn: any): Promise<void> {
  ...
  private async deleteMatrixAndRows(
    initialQuery: string,
    conn: any,
    matrixid: string,
    ...
    if (records.length > 0) {
      ...
      await this.updateOldCalMatrixVersion(matrixid, conn);
    } else {
